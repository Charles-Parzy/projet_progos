...
int do_delete(const char* picture_name, struct pictdb_file* db_file)
{
    ...
    while(strcmp(picture_name, db_file->metadata[index].pict_id) != 0 && index < db_file->header.max_files) {
        index += 1;
        while(db_file->metadata[index].is_valid == EMPTY) { //Skip the non valid metadatas
            index += 1;
        }
    }
