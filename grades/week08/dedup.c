...
{
    /* Correcteur : normalement ne devrait jamais arriver */
    if(db_file == NULL) {
    ...
    for(size_t i = 0; i < db_file->header.max_files; i++) {
        if(i != index && db_file->metadata[i].is_valid == NON_EMPTY) {
            ...
            if(!strncmp(db_file->metadata[i].pict_id, db_file->metadata[index].pict_id, MAX_PIC_ID+1)) {
                return ERR_DUPLICATE_ID;
            }

            //if the two SHA are equals.
            if(!compare_SHA(db_file->metadata[i].SHA, db_file->metadata[index].SHA)) {
