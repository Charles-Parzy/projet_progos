...
int do_insert(const char* img, const size_t size, const char* pict_id, struct pictdb_file* db_file)
{
    ...
    //correcteur: cela devrait être fait dans dedup
    ...
    //correcteur: il faudrait aussi remettre les autres champs de la structure à 0 afin d'éviter de garder du 
    //garbage des images précédentes
