...
{
    //correcteur: cette fonction ne sert presque a rien (équivalent a un free), on ne set a null que la variable locale.
    if(ptr != NULL) {
        free(ptr);
        ptr = NULL;
    }
...
*/
//correcteur: cette fonction est bien mieux, il faudrait utiliser celle la a la place de la précédente.
...
{
    //correcteur: vérifier le retour de do_list
...
{
    //correcteur: pas besoin d'allouer dynamiquement.
    ...
    if(tmp == NULL) {
        ...
        for(size_t i = 0; i < MAX_QUERY_PARAM; i++) {
            //correcteur: vérifier que les indice pair (les autres sont du data)
            if(result[i] != NULL && i+1 < MAX_QUERY_PARAM) {
                if(!strncmp(result[i], "res", strlen("res"))) { //Peut-être factoriser le i+1 < MAX_QUERRY...?
                    ...
                    //correcteur: ici result[i] peut être NULL.
                    if(result[i+1] != NULL) {
...
static void handle_insert_call(struct mg_connection *nc, struct http_message *mssg)
{
    ...
    const char* img;
    size_t size = 0;
    //correcteur: le data peut être sur plusieurs chunck et dans ce cas on ne le lit pas en entier.
...
{
    //correcteur: pas besoin de l'allouer, un tableau static était mieux adapté.
    char* tmp = calloc(MAX_QUERY_PARAM, MAX_PIC_ID+1);
    if(tmp == NULL) {
        mg_error(nc, ERR_OUT_OF_MEMORY);
    } else {
        ...
        split(result, tmp, mssg->query_string.p, delim, len);
        //We get resolution and pict_id from the query.
        //correcteur: c'est sensé être la premiere requette et il n'est pas sensé y en avoir d'autres.
        for(size_t i = 0; i < MAX_QUERY_PARAM; i++) {
            //correcteur: dans ce cas utiliser strcmp puisque on sait que ca s'arretera a la fin de pict_id comme c'est une chaine de char qu'on fourni tel quelle.
...
int main(int argc, char* argv[])
{
    if (argc < 2) {
        ...
        if(check != 0) {
            //correcteur: si do_open est bien fait, on ne devrait pas avoir a appeler do_close ici (un peu comme on appel pas free si malloc fail)
            do_close(&db_file);
            return check;
        }
        //Print the header
        ...
        //correcteur: itérer tant que s_sig_num == 0 (sinon il y a eu intéruption).
        for (;;) {
