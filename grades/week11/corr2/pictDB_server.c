...
static void handle_list_call(struct mg_connection *nc)
{
    ...
    // corr not sending the connection flags for send_and_close -1
}
...
static void handle_insert_call(struct mg_connection *nc, struct http_message *mssg)
{
    ...
    // corr -2 need to do multi-round parsing (with a while loop), using the return code of this call
