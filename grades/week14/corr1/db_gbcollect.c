...
* @return 0 or an error code if an error occured
*/
//correcteur: le db_file ne sert a rien puisqu'on peut le déterminer avec uniquement le filename.
int do_gbcollect(struct pictdb_file* db_file, const char* filename, const char* temp_filename)
{
    ...
    for(int i=0; i < db_file->header.max_files; i++) {
        if(db_file->metadata[i].is_valid == NON_EMPTY) {
            ...
            if(check != 0) {
                do_close(&db_temp);
                //correcteur: il ne faut pas libérer picture ici (il est NULL donc ca segfault)
            ...
            free_picture(&picture);
            //correcteur: le code en dessous ressemble a un copié collé.
            //If the metadata has the small image, we add it too in db_temp.
            //correcteur: il faut vérifier que l'image a la résolution donné n'est pas dans la nouvele database.
            if(db_file->metadata[i].offset[RES_SMALL] != 0) {
                check = do_read(db_file->metadata[i].pict_id, RES_SMALL, &picture, &pict_size, &db_temp);
                //correcteur: on dirait que vous ne copiez pas l'image dans la nouvelle database.
                if(check != 0) {
                    do_close(&db_temp);
                    free_picture(&picture);
                    remove(temp_filename);
