...
int do_gc_cmd(int args, char* argv[])
{
    ...
    //Open the file in Read and write
    //correcteur: il aurait été plus logique d'ouvrir ce fichier dans do_gbcollect.
    int check = do_open(dbfilename, "rb+", &db_file);
    if(check != 0) {
        do_close(&db_file);//correcteur: si do_open était bien fait on aurait pas besoin d'appeler do_close ici (penser a malloc/free et fopen/fclose, on ne ferme pas si ca a écoué).
        return check;
    }

    check = do_gbcollect(&db_file, dbfilename, temp_filename);
    //correcteur: tout le code qui suit est équivalent a :
    //do_close(&db_file);return check;
    if(check != 0) {
        do_close(&db_file);
        return check;
    }
